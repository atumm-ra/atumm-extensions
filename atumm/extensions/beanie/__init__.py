...
class BeanieLifespan(AsyncContextManager):
    ...
    def __init__(self, client: AsyncIOMotorClient, db_name: str, documents: List[Document]):
        self.beanie_client = client
        self.db_name = db_name
        self.documents = documents
    
    async def __aenter__(self) -> AsyncIOMotorDatabase:
        return await init_my_beanie(client=self.beanie_client, db_name=self.db_name, documents=self.documents)

    async def __aexit__(self, exc_type: Optional[Type[BaseException]], exc_value: Optional[BaseException], traceback: Any) -> None:
